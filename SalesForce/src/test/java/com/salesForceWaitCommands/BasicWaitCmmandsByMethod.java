...
@Test(priority = 2)
	public void invisibilityOfElementLocated() {
		/*
		 * Below is the syntax which is used for checking that an element is either invisible or not 
		 * present on the DOM.
		 */
	...
	if (element) {//also can write element==true, because by default it is true.
	...
	public void presenceOfElementLocated() {
		 ...
		 * That does not necessarily mean that the element is visible. ExpectedConditions will 
		 * return true once the element is found in the DOM. 
		 */
		 wait=new WebDriverWait(driver, 15);
